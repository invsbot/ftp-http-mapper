...
import {
  TextField, Button, Grid, List, Tabs, Tab, ListItem, ListItemText, Typography,
...
const ParserPage: React.FC<ParserPageProps> = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(formParams).length !== 2) { return; }

    try {
      const response = await fetch('/mapper',
        {
          mode: 'no-cors',
          method: 'POST',
          headers: {
            ...
            <>
              ...
              <TabPanel value={tabPanelPosition} index="one">
                <p className="page-size-count">
                  Общее количество:
                  {' '}
                  {parsedUrls.reduce((acc, pageItem) => acc + Number(pageItem.pageSize), 0)}
                </p>
